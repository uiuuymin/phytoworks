...
| `pricing` | 가격 표시값, 통화, 출처와 권위 여부 | Current Demo — NITRO는 카탈로그 참고값, 두 모듈은 UI 검증용 Demo값 |
| `optionGroups` | 카탈로그에 기재된 선택 가능 구성 후보 | Current Demo — NITRO의 Depth, 관수와 추가 옵션 |
...
- **Current Demo:** Web은 가격 object의 `authoritative: false`로 위 경계를 보존하며, 실제 결제 금액이나 주문 단가로 사용하지 않는다.
...
- **Current Demo:** 카탈로그 옵션은 Product Detail에서 읽기 전용 구성 후보로 표시한다. 옵션 선택 저장, 옵션별 추가 금액과 견적 전송은 아직 구현하지 않는다.
